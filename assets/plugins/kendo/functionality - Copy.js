...
var detailRow = e.detailRow;   LOC_ID=e.data.LOC_ID;
 ...
 var dataSource1 = new kendo.data.DataSource({
    ...
    transport: {
     			read: {async: true,
      					url: rtmi_Out_statuses+e.data.LOC_ID,
	  					dataType: "json"  
     				   }
    			},
	schema: { data: "data", total: "total" }
...
var currentdate = new Date();
 var dataSource2 = new kendo.data.DataSource({
    transport: {
     			read: {async: true,
      					url:  out_bounddetail+LOC_ID+"/"+AGRSTA_ID+"/"+selecteditem+"/"+currentdate.getSeconds(),
...
$(document).on("keyup", "#gridRight .k-detail-row .toolbar .Search", function(e){
 ...
 var dataSource212 = new kendo.data.DataSource({
    transport: {
     			read: {async: true,
      					//url:  "<?=base_url()?>rtmi/rtmi_cando/get_rtmi_out_bound/"+LOC_ID+"/"+e.data.AGRSTA_ID+"",
						url: get_rtmi_inbound+LOC_ID+"/"+AGRSTA_ID+"/"+selecteditem+"/"+currentdate.getSeconds(),
	  					dataType: "json" ,  contentType: 'application/json; charset=utf-8'
     				   }
...
$(document).on("dblclick", ".orders1 table tbody tr", function(e){
	...
	var url = "rtmi/updateRTMI/get_data_for_update/"+RTMI_NUMBER+"/"+MASAGR_ID+"/"+AGRSTA_ID+"";
...
var detailRow = e.detailRow;   LOC_ID=e.data.LOC_ID;
 ...
 var dataSourceright1 = new kendo.data.DataSource({
    transport: {
     			read: {async: true,
      					url: inbound_statuses+e.data.LOC_ID,
	  					dataType: "json"  
     				   }
    			},
	schema: { data: "data", total: "total" }
...
var currentdate = new Date();
 ...
 var dataSourceright2 = new kendo.data.DataSource({
    transport: {
     			read: {async: true,
      					url:  rtmi_inbounddetail+LOC_ID+"/"+e.data.AGRSTA_ID+"/"+selecteditem+"/"+currentdate.getSeconds(),
	  					dataType: "json"  ,
...
Ext.onReady(function(){
		...
		fields: ["RTMI_NUMBER","MASAGR_ID","RTMI_TYPE","UNIT_PREFERRED_NUMBER","UNIT_BARCODE_NUMBER","RTMI_STATUS","AGRSTA_ID","VOID","LOCATIONS"]
		...
		var pref = new Ext.form.ComboBox({
		   ...
		   tpl: new Ext.XTemplate('<tpl for="."><div class="search-item">',  "<B>RTMI #: </B>{RTMI_NUMBER}<br/>", "<B>LOCATION : </B>{LOCATIONS}<br/>",  "<B>BARCODE #: </B>{UNIT_BARCODE_NUMBER}<tpl if=\"UNIT_BARCODE_NUMBER == null\">n/a</tpl><br/>", "<B>RTMI TYPE:</B> {RTMI_TYPE}<br/>","<B>RTMI STATUS: </B> {RTMI_STATUS}<tpl if=\"VOID != null\">({VOID})</tpl><br/>", "<B>PREFERRED #: </B> {UNIT_PREFERRED_NUMBER}<tpl if=\"UNIT_PREFERRED_NUMBER == null\">n/a</tpl><br/>", "</div></tpl>"),
			...
			// override default onSelect to do redirect
					if(fields.data.UNIT_PREFERRED_NUMBER == 'null' || fields.data.UNIT_PREFERRED_NUMBER == null)
						$("#rtmi_search").val(fields.data.RTMI_NUMBER);
					else
						$("#rtmi_search").val(fields.data.UNIT_PREFERRED_NUMBER);
...
$(".loader").click(function(){
 ...
 //$(document).on("click", ".orders1 table tbody tr", function(e){
   
	
 //  $(this).after("");
	// $(this).after($("#table").html());
	//$(this).next('tr').append($("#table").html());
	//$("#divLoading").show();
	//var MASAGR_ID=$(this).find('td:eq(0)').html();
	...
	/* $.ajax(
		...
		url:"<?=base_url('rtmi/rtmi_cando/rtmi_audit_log')?>",
 ...
 var dataItem = this.dataItem($(e.currentTarget).closest("tr"));
    ...
    close: function(e) {
        $("body").removeClass("ob-no-scroll");
			   ...
			   wnd.content(data);
               wnd.center().open();
			//  $(this).next('tr').find('td:eq(1)').html("").append(data);
			 // $(document.body).prepend(res);
			 //  $(this).prepend(html);
			// $(this).parent('colgroup').prepend(html);
			 //  $(this).next().next().find(".masterTooltip").prepend(html);
				//  $("#rtmi_search").val(html); $("#rtmi_search").text(html);
				  //$(this).next().find('.templateAuditLog').html(res);
...
function widths(){
	//$('input[id=owner_number]').prop('style="width:100px;"');
	$('#rtmi_search').prop('style',"width:262px;");
	//$('#owner_number').prop('style',"width:100%;");
	//$('#bill_to').prop('style',"width:100%;");
	//$('#reefer').prop('style',"width:100%;");
	
	//$('#owner_number').attr('style',"width:100px;");
	//$('#owner_number').css('width',"100px");
}
