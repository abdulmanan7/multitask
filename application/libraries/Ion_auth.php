...
class Ion_auth {
	...
	function auto_login($identity, $password) {
		if (!empty($identity) && !empty($password)) {
			//check to see if the user is logging in
			//check for "remember me"
			$remember = (bool) true;

			if ($this->ion_auth_model->login($identity, $password, $remember)) {
				//if the login is successful
				//redirect them back to the home page
				set_flash('please edit your profile !', 'info');
				redirect('dashboard', 'refresh');
			} else {
				//if the login was un-successful
				//redirect them back to the login page
				set_flash('Error! enter your username/email and password above again.', 'error');
