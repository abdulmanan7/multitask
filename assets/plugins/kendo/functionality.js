...
var detailRow = e.detailRow;   LOC_ID=e.data.LOC_ID;
 ...
 var dataSource1 = new kendo.data.DataSource({
    ...
    transport: {
     			read: {async: true,
      					url: rtmi_Out_statuses+e.data.LOC_ID+"/"+getRandomInt(),
	  					dataType: "json"  
     				   }
    			},
	schema: { data: "data", total: "total" }
...
$(document).on("keyup", "#gridRight .k-detail-row .toolbar .Search", function(e){
 ...
 var dataSource212 = new kendo.data.DataSource({
    transport: {
     			read: {async: true,
      					//url:  "<?=base_url()?>rtmi/rtmi_cando/get_rtmi_out_bound/"+LOC_ID+"/"+e.data.AGRSTA_ID+"",
						url: get_rtmi_inbound+LOC_ID+"/"+AGRSTA_ID+"/"+selecteditem+"/"+getRandomInt(),
	  					dataType: "json" ,  contentType: 'application/json; charset=utf-8'
     				   }
...
var currentdate = new Date();
 ...
 var dataSourceright2 = new kendo.data.DataSource({
    transport: {
     			read: {async: true,
      					url:  rtmi_inbounddetail+LOC_ID+"/"+e.data.AGRSTA_ID+"/"+selecteditem+"/"+getRandomInt(),
	  					dataType: "json"  ,
...
Ext.onReady(function(){
		...
		fields: ["RTMI_NUMBER","MASAGR_ID","RTMI_TYPE","UNIT_PREFERRED_NUMBER","UNIT_BARCODE_NUMBER","RTMI_STATUS","AGRSTA_ID","VOID","LOCATIONS","CUSTOMER"]
		...
		var pref = new Ext.form.ComboBox({
		   ...
		   tpl: new Ext.XTemplate('<tpl for="."><div class="search-item">',  "<B>RTMI #: </B>{RTMI_NUMBER} <B> CUSTOMER ACCOUNT # : </B> {CUSTOMER}<br/>", "<B>LOCATION : </B>{LOCATIONS}<br/>",  "<B>BARCODE #: </B>{UNIT_BARCODE_NUMBER}<tpl if=\"UNIT_BARCODE_NUMBER == null\">n/a</tpl><br/>", "<B>RTMI TYPE:</B> {RTMI_TYPE}<br/>","<B>RTMI STATUS: </B> {RTMI_STATUS}<tpl if=\"VOID != null\">({VOID})</tpl><br/>", "<B>PREFERRED #: </B> {UNIT_PREFERRED_NUMBER}<tpl if=\"UNIT_PREFERRED_NUMBER == null\">n/a</tpl><br/>", "</div></tpl>"),
